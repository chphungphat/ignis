...
const RouteConfigs = {
  INFO: {
    ...
    responses: jsonResponse({
      schema: WSInfoSchema,
      description: 'WebSocket server info',
    }),
  },
  CLIENTS: {
    method: HTTP.Methods.GET,
...
export class WebSocketTestController extends BaseController {
  constructor(
    ...
    super({ scope: WebSocketTestController.name });
  ...
  binding() {
    ...
    this.bindRoute({ configs: RouteConfigs.INFO }).to({
      handler: (c: TRouteContext) => {
        const clients = this.wsEventService.getConnectedClients();
    ...
    this.bindRoute({ configs: RouteConfigs.BROADCAST }).to({
      handler: async (c: TRouteContext) => {
        ...
        this.wsEventService.broadcastMessage({ topic, data });

        return c.json(
          {
            success: true,
            message: `Broadcast sent to ${this.wsEventService.getClientCount()} clients`,
          },
          HTTP.ResultCodes.RS_2.Ok,
        );
      },
    ...
    this.bindRoute({ configs: RouteConfigs.SEND_TO_ROOM }).to({
      handler: async (c: TRouteContext) => {
        ...
        this.wsEventService.sendToRoom({ room: roomId, topic, data });
    ...
    // POST /ws-api/client/:clientId/send
    this.bindRoute({ configs: RouteConfigs.SEND_TO_CLIENT }).to({
      handler: async (c: TRouteContext) => {
        ...
        this.wsEventService.sendToClient({ clientId, topic, data });
    ...
    this.bindRoute({ configs: RouteConfigs.SEND_TO_USER }).to({
      handler: async (c: TRouteContext) => {
        ...
        this.wsEventService.sendToUser({ userId, topic, data });
    ...
    // POST /ws-api/client/:clientId/join
    this.bindRoute({ configs: RouteConfigs.JOIN_ROOM }).to({
      handler: async (c: TRouteContext) => {
        ...
        const result = this.wsEventService.joinRoom({ clientId, rooms });

        return c.json(result, HTTP.ResultCodes.RS_2.Ok);
      },
    });

    // POST /ws-api/client/:clientId/leave
    this.bindRoute({ configs: RouteConfigs.LEAVE_ROOM }).to({
      handler: async (c: TRouteContext) => {
        ...
        const result = this.wsEventService.leaveRoom({ clientId, rooms });

        return c.json(result, HTTP.ResultCodes.RS_2.Ok);
      },
    });

    // GET /ws-api/client/:clientId/rooms
    ...
    this.bindRoute({ configs: RouteConfigs.HEALTH }).to({
      handler: (c: TRouteContext) => {
        return c.json(
          {
            healthy: this.wsEventService.isReady(),
